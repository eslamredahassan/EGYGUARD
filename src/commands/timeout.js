...
module.exports = async (client, config) => {
  ...
  client.on("interactionCreate", async (interaction) => {
    if (interaction.isCommand() && interaction.commandName === "timeout") {
      ...
      if (
        !guild.members.me.permissions.has(Permissions.FLAGS.TIMEOUT_MEMBERS)
      ) {
        return interaction.editReply({
          content: "I don't have the TIMEOUT_MEMBERS permission!",
          ephemeral: true,
        });
      }
      ...
      try {
        ...
        if (targetUser.isCommunicationDisabled()) {
          ...
          await interaction.editReply({
            content: `${targetUser}'s timeout has been updated to ${prettyMs(
              msDuration,
              { verbose: true },
            )}\nReason: ${reason}`,
            ephemeral: true,
        ...
        await targetUser.timeout(msDuration, reason);
        await interaction.editReply({
          content: `${targetUser} was timed out for ${prettyMs(msDuration, {
            verbose: true,
          })}. Reason: ${reason}`,
          ephemeral: true,
        });
